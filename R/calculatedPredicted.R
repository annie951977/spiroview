...
#'
#' Assumes that df is a dataframe that contains at least the age and the height
#' of the patient
#' @param df A dataframe with colnames at the minimum age and height
#' @return
#' @examples
#' @import rspiro
calculateLLNPret <- function(df, param = "FEV1", ref = "GLI") {
  ...
  if (ref == "NHANESS3") {
    return(rspiro::LLN_NHANES3(age, height, gender, eth, param))
  } else {
...
#' @examples
#' @import rspiro
calculateMeanPret <- function(df, param = "FEV1", ref = "GLI") {
  # if the df has age, height, gender
  if(is.null(df$age) || is.null(df$height)){
    stop("Not enough demographic parameters to calculate mean predicted")
  }
